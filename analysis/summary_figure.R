...
p_experiment <- drop_na(s_variants, mut_escape_max) %>%
  ...
  geom_text_repel(data = interesting_variants, colour = 'black', show.legend = FALSE, size = 1.8, force = 4, max.overlaps = Inf) +
...
p2 <- p_sift_freq + labs(tag = 'B') + size
p3 <- p_foldx_dms + labs(tag = 'C') + size
p4 <- p_int_dms + labs(tag = 'D') + size
p5 <- p_roc + labs(tag = 'E') + size
p6 <- p_experiment + labs(tag = 'F') + size
p7 <- p_structure + labs(tag = 'G') + size

figure <- multi_panel_figure(width = 180, height = 180, rows = 3, columns = 3,
